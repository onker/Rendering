...
void RenderEngine::init_view()
{
  ...
  // Initialize corresponding era for tracer
...
void RenderEngine::render()
{
  ...
  for(int y = 0; y < static_cast<int>(res.y); ++y)
  {
    ...
    // tracer  (ray tracer with access to the function compute_pixel)
	  ...
	  {
		   float3 result = tracer.compute_pixel(x, y);
		   image[static_cast<int>(res.x)*y + x ] = result;
	  }
    if(((y + 1) % 50) == 0) 
      cerr << ".";
  }
...
// sample(const float3d position, float3d dir, float3d Le)
// pos = intersection
